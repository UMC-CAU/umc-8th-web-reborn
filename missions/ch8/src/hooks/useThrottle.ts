...
// 최소 interval(밀리초) 간격으로만 업데이트해서 성능을 개선
...
function useThrottle<T>(value: T, delay: number) {
  ...
  useEffect(() => {
    // 현재 시각과 lastExecuted.current에 저장된 마지막 시각 + interval을 비교합니다.
    // 충분한 시간이 지나면 바로 업데이트
    if (Date.now() >= lastExecuted.current + delay) {
      ...
      // 충분한 시간이 지나지 않았다면, delay 시간 후에 업데이트(최신 value로)
      ...
      // CleanUp Function : 타이머가 만료되기 전에 컴포넌트가 언마운트되면, 타이머를 취소합니다.

...
export const GoogleLogin = ({ /* onSuccess, */ onError }: GoogleLoginProps) => {
  ...
  useEffect(() => {
    ...
    // 필수 환경 변수 체크
    if (!googleClientId || !redirectUri || !serverApiUrl) {
      const errorMsg = "Google 인증에 필요한 환경 변수(Client ID, Redirect URI, Server API URL)가 설정되지 않았습니다. 관리자에게 문의하세요.";
  ...
  //           ? response.access_token.substring(0, 5) + "..."
  //           : "없음",
  //       });
  ...
  const handleGoogleLogin = () => {
    try {
      ...
      const serverApiUrl = import.meta.env.VITE_SERVER_API_URL;
      if (serverApiUrl) {
        console.log("백엔드 Google 로그인 엔드포인트로 리디렉션");
        window.location.href = `${serverApiUrl}/v1/auth/google/login`;
      } else {
        const errorMsg = "백엔드 서버 API URL이 설정되지 않았습니다.";
        console.error(errorMsg);
        onError?.(errorMsg);
      }
